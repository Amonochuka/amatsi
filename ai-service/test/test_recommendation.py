...
def test_rain_has_highest_priority_and_saves_water():
    result = generate_recommendation(
        RecommendationRequest(rainfall_probability=70, soil_moisture=10, tank_capacity_liters=100)
    )
...
def test_dry_soil_recommends_irrigation_with_crop_water_need():
    result = generate_recommendation(
        RecommendationRequest(crop_type="maize", field_size_square_m=10, soil_moisture=29)
    )
...
def test_adequate_soil_moisture_recommends_monitoring():
    result = generate_recommendation(RecommendationRequest(soil_moisture=30))
    assert result.action == "MONITOR"


def test_low_tank_conserves_when_earlier_rules_do_not_match():
    result = generate_recommendation(
        RecommendationRequest(soil_moisture=70, tank_capacity_liters=499)
    )
